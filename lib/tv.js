                    ...
                    Promise.all(
                        episode.each(async function () {
                            ...
                            const id = link
                                ...
                                const episode_text = id.replaceAll("-", " ").match(/season (\d+) episode (\d+)/gmi) ? id.replaceAll("-", " ").match(/season (\d+) episode (\d+)/gmi)[0] : null

                            if ($(this).text() !== "Info") {
                                console.log(episode_text)
                                list.push({ episode: episodeList,episode_text, link, id, download: [], embed: [] });
                            }
                        })
                    );
                    //console.log(list);
