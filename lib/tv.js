...
class Tv {
    ...
    async meta(url) {
        ...
        const cachedData = await this.checkCache('tv/' + path);
        if (cachedData) {
        ...
        await this.saveCache('tv/' + path, data, true);

        return data;
    }
    async detail(id) {
        const cachedData = await this.checkCache('tv/details/' + id);
        if (cachedData) {
          console.log('Data ditemukan dalam cache');
          return cachedData;
        }
        ...
        */
        ...
        await this.saveCache('tv/details/' + id, result);
        return result;

        //await this.fetchEpisodes(result)
    }
    async show(id) {
        const cachedData = await this.checkCache('tv/show/' + id);
        if (cachedData) {
          console.log('Data ditemukan dalam cache');
          return cachedData;
        }
        ...
        */
          ...
          await this.saveCache('tv/show/' + id, final);
        return final
    }
