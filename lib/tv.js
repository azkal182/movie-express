...
class Tv {
  ...
  async meta(url) {
    ...
    /*
    const cachedData = await this.checkCache("tvs/" + path);
    if (cachedData) {
    ...
    */
    ...
    //await this.saveCache("tvs/" + path, data, true);

    return data;
  }
  async detail(id) {
    const cachedData = await this.checkCache("tvs/details/" + id);
    if (cachedData) {
      console.log("Data ditemukan dalam cache");
      return cachedData;
    }
    ...
    const result = await axios(url + id).then(({ data: html }) => {
      ...
      tes.each(function () {
        ...
        if (
          ...
          $(this)
            .text()
            .match(/(?<=Bintang film)(.*)/gm)
        ) {
          let cast = $(this)
            .text()
            .match(/(?<=Bintang film)(.*)/gm)[0]
            .split(",");
          cast.forEach((i) => {
      ...
      const srcOverview = $(
        "#movie-detail > div > div.col-xs-9.content > blockquote"
      ).html();
      let overview = srcOverview ? srcOverview.split("<br>")[1] : "";
        ...
        episodeList.each(async function (i, v) {
          ...
          Promise.all(
            episode.each(async function () {
              ...
              const episode_text = id
                .replaceAll("-", " ")
                .match(/season (\d+) episode (\d+)/gim)
                ? id
                    .replaceAll("-", " ")
                    .match(/season (\d+) episode (\d+)/gim)[0]
                : null;

              if ($(this).text() !== "Info") {
                // console.log(episode_text);
                list.push({
                  episode: episodeList,
                  episode_text,
                  link,
    ...
    await this.saveCache("tvs/details/" + id, result);
    return result;

    //await this.fetchEpisodes(result)
  }
  async show(id) {
    const cachedData = await this.checkCache("tvs/show/" + id);
    if (cachedData) {
      console.log("Data ditemukan dalam cache");
      return cachedData;
    }
    ...
    let get_download = await axios({
      method: "post",
      url: "https://selfviolet.xyz/verifying.php",
      data: {
        slug: query,
      },
      headers: {
    ...
    await this.saveCache("tvs/show/" + id, final);
    return final;
  }
  ...
  async getCookie(id) {
    ...
    let result = await axios
      .get("https://selfviolet.xyz/get/" + id, config)
      .then((res) => {
        let data = res.data;
