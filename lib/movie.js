...
const url = process.env.HOST_MOVIE ?? "https://tv5.lk21official.wiki/";
...
/*
    const cachedData = await this.checkCache("movies/" + path);
    if (cachedData) {
...
*/
    ...
    let data = await axios.get(url, config).then((res) => {
      ...
      item.each(function (i, v) {
        ...
        let id = $(this)
          .find("article > footer > p:nth-child(3) > a")
          .attr("href");
        //console.log(id)
   ...
   // await this.saveCache("movies/" + path, data, true);
  ...
  async detail(id) {
    ...
    */
    ...
    let result = await axios.get(url + query, config).then((res) => {
      ...
      data.overview = get_meta.find("blockquote").html()
        ? get_meta.find("blockquote").html().split("<br>")[1].trim()
        : "";
    ...
    let get_download = await axios({
      method: "post",
      url: "https://dl.lk21static.xyz/verifying.php?"+"slug="+query,
      data: {
        slug: query,
      },
      headers: {
...
/*
    if (final.results.length > 0) {
      await this.saveCache("movies/detail/" + id, final);
    }
*/
    return final;
  }
  ...
  async getCookie(id) {
    ...
    let result = await axios
      .get("https://dl.lk21static.xyz/get/" + id, config)
      .then((res) => {
        let data = res.data;
